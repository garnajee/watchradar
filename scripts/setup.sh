...
prompt_optional() {
  ...
  if [[ -n "$current" ]]; then
    ...
    read -r -p "$label [configure later in the browser]: " value
    printf '%s' "$value"
  fi
}
...
if [[ -z "$jellyfin_url" ]]; then
  if [[ "$non_interactive" == true ]]; then
    ...
    jellyfin_url=$(prompt_optional "Jellyfin HTTPS URL" "$existing_jellyfin_url")
  fi
fi
jellyfin_url=$(trim_trailing_slashes "$jellyfin_url")
[[ -z "$jellyfin_url" ]] || is_jellyfin_https_url "$jellyfin_url" ||
...
jellyfin_api_key=$(strip_quotes "$(get_env_value JELLYFIN_API_KEY)")
[[ -z "$jellyfin_api_key" ||
  (${#jellyfin_api_key} -ge 8 && ${#jellyfin_api_key} -le 500 &&
    "$jellyfin_api_key" =~ ^[a-zA-Z0-9._~-]+$) ]] ||
  die "JELLYFIN_API_KEY must be empty or contain 8 to 500 safe characters"
...
set_env_value "$working_file" JELLYFIN_API_KEY "$jellyfin_api_key"
...
printf 'Public URL: %s\n' "$public_url"
printf 'TLS certificates are managed by the reverse proxy.\n'
printf 'The optional Jellyfin URL and API key can be added later in WatchRadar.\n'
